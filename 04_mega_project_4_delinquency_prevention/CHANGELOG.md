...
|---|---|
| [1.9.9] | Problems 3-6 have since been run end-to-end on your own real, full-scale data (2026-09-02) -- real dashboards (233KB-294KB) and summary JSONs with the real 307,511-applicant population now exist under each notebook's `decision_engine/reports/`. The [1.9.3] policy-change disclosure (hand-built test cases, no fixture run) described how these were verified *before delivery*; `README.md` corrected to also state they are no longer only hand-built-test-verified -- they have real, full-scale execution evidence now, on the same footing as Problems 1-2. No `sample_reports/SAMPLE_*` fixture demo files exist for Problems 3-6 still (that artifact type was never generated for them), which is a separate, narrower fact from execution status. |
