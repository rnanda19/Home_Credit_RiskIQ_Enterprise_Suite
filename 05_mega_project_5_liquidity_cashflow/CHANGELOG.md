...
|---|---|
| [2.1.1] | Real deployable FastAPI service for Problem 4 (`services/prepayment_segment_assignment_service.py`, port 8015), Docker packaging (`docker/`), and a real integration test (`tests/test_scoring_services.py`, skips cleanly until the bundle exists) all added. Architecture diagram added (`docs/mp5_architecture_flow.mmd`/`.png`). `ci.yml`/`code-quality.yml`/`Makefile` now cover this Mega Project's `services/`/`tests/`. Still pending: re-running Notebook 04 to produce the real `.joblib` bundle so the service can be verified. |
