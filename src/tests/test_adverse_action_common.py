...
def test_protected_basis_never_appears_in_customer_reasons():
    ...
    notice = render_adverse_action_notice(top_reasons, score=0.8, threshold=0.5, adverse_direction="positive")
    ...
    category = next(s["category"] for s in notice["suppressed_factors"] if s["factor"] == "CODE_GENDER")
...
def test_fair_lending_proxy_suppressed_with_correct_category():
    ...
    notice = render_adverse_action_notice(top_reasons, score=0.9, threshold=0.5, adverse_direction="positive")
...
def test_only_factors_that_worked_against_applicant_are_reasons_positive_direction():
    ...
    notice = render_adverse_action_notice(top_reasons, score=0.7, threshold=0.5, adverse_direction="positive")
    factors_shown = [r["factor"] for r in notice["principal_reasons"]]
...
def test_negative_direction_flips_which_sign_is_adverse():
    ...
    top_reasons = [
        {"factor": "AMT_INCOME_TOTAL", "contribution": -0.30},  # lowered approval prob -> adverse
        {"factor": "EXT_SOURCE_2", "contribution": 0.20},  # raised approval prob -> helped
    ]
    notice = render_adverse_action_notice(top_reasons, score=0.2, threshold=0.5, adverse_direction="negative")
    assert notice["decision"] == "adverse_action"
    factors_shown = [r["factor"] for r in notice["principal_reasons"]]
...
def test_approved_case_returns_no_reasons_even_if_factors_exist():
    ...
    notice = render_adverse_action_notice(top_reasons, score=0.3, threshold=0.5, adverse_direction="positive")
...
def test_max_reasons_caps_the_list():
    top_reasons = [
        ...
        for i, f in enumerate(
            ["AMT_CREDIT", "AMT_ANNUITY", "AMT_INCOME_TOTAL", "DAYS_EMPLOYED", "EXT_SOURCE_1"]
        )
    ]
    notice = render_adverse_action_notice(
        top_reasons, score=0.9, threshold=0.5, adverse_direction="positive", max_reasons=3
    )
...
def test_invalid_adverse_direction_raises():
    try:
        render_adverse_action_notice([], score=0.5, threshold=0.5, adverse_direction="sideways")
