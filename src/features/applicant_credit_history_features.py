...
def engineer_bureau_history_features(
    bureau: pl.DataFrame, bureau_balance: pl.DataFrame
...
def engineer_prev_loan_servicing_features_loo(
    ...
    all_curr_ids = (
        pos_curr.select("SK_ID_CURR")
        .vstack(inst_curr.select("SK_ID_CURR"))
        .vstack(cc_curr.select("SK_ID_CURR"))
        .unique()
    )
    totals_df = (
    ...
    all_prev_ids = (
        pos_prev.select("SK_ID_PREV")
        .vstack(inst_prev.select("SK_ID_PREV"))
        .vstack(cc_prev.select("SK_ID_PREV"))
        .unique()
    )
    own_df = (
...
def engineer_previous_application_features(
