...
def engineer_revolving_distress_features(credit_card: pl.DataFrame) -> tuple[pl.DataFrame, list[str]]:
    ...
    base = base.with_columns(
        [
            (pl.col("_UTIL") - pl.col("_UTIL").shift(1).over("SK_ID_CURR"))
            .fill_null(0.0)
            .alias("_UTIL_JUMP"),
        ]
    )
    ...
    longest_min_pay = streaks.group_by("SK_ID_CURR").agg(
        [
            pl.col("STREAK_LEN")
            .filter(pl.col("STREAK_IS_MIN_PAY_ONLY"))
            .max()
            .fill_null(0)
            .alias("LONGEST_MIN_PAYMENT_ONLY_STREAK"),
        ]
    )
    current_min_pay = (
    ...
    recent = half_agg.filter(pl.col("_IS_RECENT_HALF")).select(
        [
            "SK_ID_CURR",
            pl.col("_MEAN_BALANCE").alias("_BAL_RECENT"),
            pl.col("_MEAN_DRAWINGS").alias("_DRAW_RECENT"),
        ]
    )
    early = half_agg.filter(~pl.col("_IS_RECENT_HALF")).select(
        [
            "SK_ID_CURR",
            pl.col("_MEAN_BALANCE").alias("_BAL_EARLY"),
            pl.col("_MEAN_DRAWINGS").alias("_DRAW_EARLY"),
        ]
    )
    velocity = (
        ...
        .with_columns(
            [
                pl.col("_BAL_RECENT").fill_null(0.0),
                pl.col("_BAL_EARLY").fill_null(0.0),
                pl.col("_DRAW_RECENT").fill_null(0.0),
                pl.col("_DRAW_EARLY").fill_null(0.0),
            ]
        )
        .with_columns(
            [
    ...
    feature_cols = [
        "N_CC_MONTHS",
        "CURRENT_UTILIZATION",
        "MAX_UTILIZATION_JUMP",
        "N_UTILIZATION_SPIKE_MONTHS",
        "LONGEST_MIN_PAYMENT_ONLY_STREAK",
