...
def engineer_installment_behavior_features(
    ...
    base = installments.with_columns(
        ...
        [
            ...
            .alias("PAYMENT_RATIO"),
            pl.when(pl.col("_NO_PAYMENT_RECORDED"))
            .then(pl.col("AMT_INSTALMENT").clip(lower_bound=0))
            .otherwise(pl.col("SHORTFALL_AMT"))
            .alias("SHORTFALL_AMT"),
        ]
    )
    ...
    overall_agg = base.group_by("SK_ID_CURR").agg(
        [
    ...
    half_agg = (
        ...
        else base.group_by(["SK_ID_CURR", "_IS_RECENT_HALF"]).agg(
            pl.col("IS_LATE").mean().alias("_LATE_RATE")
        )
    )
    recent = half_agg.filter(pl.col("_IS_RECENT_HALF")).select(
        ["SK_ID_CURR", pl.col("_LATE_RATE").alias("_LATE_RATE_RECENT")]
    )
    early = half_agg.filter(~pl.col("_IS_RECENT_HALF")).select(
        ["SK_ID_CURR", pl.col("_LATE_RATE").alias("_LATE_RATE_EARLY")]
    )
    trend = (
    ...
    feat = overall_agg.join(trend, on="SK_ID_CURR", how="left").with_columns(
        [
    ...
    feature_cols = [
        "N_INSTALLMENTS",
        "N_PREV_LOANS_SERVICED",
        "PCT_INSTALLMENTS_LATE",
        "MEAN_DAYS_LATE",
        "MAX_DAYS_LATE",
        "STD_DAYS_LATE",
        "MEAN_DAYS_LATE_WHEN_LATE",
        "PCT_INSTALLMENTS_UNDERPAID",
        "MEAN_PAYMENT_RATIO",
        "MIN_PAYMENT_RATIO",
        "TOTAL_SHORTFALL_AMT",
        "LATE_RATE_TREND",
...
def engineer_payment_streak_features(
    installments: pl.DataFrame,
) -> tuple[pl.DataFrame, list[str]]:
    ...
    base = installments.sort(["SK_ID_CURR", "DAYS_INSTALMENT"]).with_columns(
        [
            (
                pl.col("DAYS_ENTRY_PAYMENT").is_null()
    ...
    base = base.with_columns(
        [
            (pl.col("IS_LATE") != pl.col("IS_LATE").shift(1).over("SK_ID_CURR"))
            .fill_null(True)
            .alias("_NEW_STREAK"),
        ]
    )
    ...
    feature_cols = [
        "LONGEST_LATE_STREAK",
        "LONGEST_ONTIME_STREAK",
        "N_LATE_STREAKS",
        "N_TOTAL_STREAKS",
        "CURRENT_STREAK_IS_LATE_INT",
        "CURRENT_STREAK_LEN",
        "ALTERNATION_RATE",
    ]
    return feat, feature_cols
